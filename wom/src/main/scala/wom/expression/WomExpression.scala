...
trait IoFunctionSet {
  ...
  def copyFile(pathFrom: String, targetName: String): Future[WomSingleFile]
  def stdout(params: Seq[Try[WomValue]]): Try[WomSingleFile]
  ...
  def size(path: String): Future[Double]
}
