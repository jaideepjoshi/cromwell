...
class Draft3SizeFunctionSpec extends FlatSpec with Matchers {
  ...
  it should "correctly report a 2048 byte file, in bytes by default" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), None).evaluateValue(Map.empty, testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in bytes" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("B")))).evaluateValue(Map.empty, testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2048d))
    }
  }

  it should "correctly report a 2048 byte file, in KB" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("KB")))).evaluateValue(Map.empty, testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2.048d))
    }
  }

  it should "correctly report a 2048 byte file, in KiB" in {
    validate(Size(PrimitiveLiteralExpressionElement(WomSingleFile("blah")), Some(PrimitiveLiteralExpressionElement(WomString("KiB")))).evaluateValue(Map.empty, testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2d))
    }
  }
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file" in {
    ...
    validate(Size(IdentifierLookup("x"), None).evaluateValue(Map("x" -> value), testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2048d))
    }
  }
  ...
  it should "correctly report the size of a supplied, optional, 2048 byte file, in MB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Success(2048d)))) {
  ...
  it should "correctly report that an unsupplied optional file is empty" in {
    ...
    validate(Size(IdentifierLookup("x"), None).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an empty optional"))))) {
      res => assert(res == WomFloat(0d))
    }
  }
  ...
  it should "correctly report that an unsupplied optional file is empty, even in MB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an empty optional"))))) {
      res => assert(res == WomFloat(0d))
    }
  }
  ...
  it should "correctly report the size of an array of files, in GiB" in {
    ...
    validate(Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("GiB")))).evaluateValue(Map("x" -> value), testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2048d * 2 / 1024 / 1024 / 1024))
    }
  }
  ...
  it should "correctly report the size of a mixed Array[File?] - some supplied and some not" in {
    ...
    validate(Size(IdentifierLookup("x"), None).evaluateValue(Map("x" -> value), testFunctions(Success(2048d)))) {
      res => assert(res == WomFloat(2048d * 2))
    }
  }

  it should "refuse to report file sizes for Ints" in {
    val value = WomInteger(55)
    val oops = Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an Int"))))
    oops match {
      case Valid(x) => fail(s"Expected an Integer to not have a file length but instead got ${x.toWomString}")
  ...
  it should "refuse to report file sizes for Int?s" in {
    ...
    val oops = Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("Bad call to size on an Int?"))))
    oops match {
      case Valid(x) => fail(s"Expected an Int? to not have a file length but instead got ${x.toWomString}")
      case Invalid(e) => e.head should be("The 'size' method expects a 'File', 'File?', 'Array[File]' or Array[File?] argument but instead got Int?.")
    }
  }
  ...
  it should "pass on underlying size reading errors" in {
    ...
    val oops = Size(IdentifierLookup("x"), Some(PrimitiveLiteralExpressionElement(WomString("MB")))).evaluateValue(Map("x" -> value), testFunctions(Failure(new Exception("'size' inner exception, expect me to be passed on"))))
    oops match {
      case Valid(x) => fail(s"Expected an Int? to not have a file length but instead got ${x.toWomString}")
...
object Draft3SizeFunctionSpec {
  def testFunctions(sizeResult: Try[Double]): IoFunctionSet = new IoFunctionSet {
    ...
    override def size(path: String): Future[Double] = Future.fromTry(sizeResult)
