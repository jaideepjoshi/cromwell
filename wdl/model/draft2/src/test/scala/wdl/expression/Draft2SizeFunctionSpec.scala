...
class Draft2SizeFunctionSpec extends FlatSpec with Matchers {
  ...
  it should "correctly report a 2048 byte file, in bytes by default" in {
    val readLike = testFunctions(Success(2048d))
...
object Draft2SizeFunctionSpec {
  def testFunctions(sizeResult: Try[Double]): WdlStandardLibraryFunctions = WdlStandardLibraryFunctions.fromIoFunctionSet( new IoFunctionSet {
